...
for (let i = 0; i < products.length; i++) { // Tsükkel jätkub kuni i väärtus on väiksem kui massiivi pikkus
    if (products[i] === "Muna" || products[i] === "Sibul" || products[i] === "Riis") { // Kui produkt on muna OR sibul OR riis, siis jätab vahele
        continue;
    }
    if (i === 10) { // Prindib kuni 10-ni
        break;
    }
    console.log(`${i + 1}. ${products[i]}`); // Lisab indeksile ühe (indeks algab nullist, seega lisades ühe, saan reaalse järjekorranumbri)
}
...
for (let i = 0; i < temperatures.length; i++) {
  let sum = 0; // algväärtus

  for (let a = 0; a < temperatures.length; a++) {
    sum += temperatures[a][i];
  }

  const keskmine = sum / temperatures.length;
  console.log(`${monthArray[i]}i keskmine temperatuur on ${keskmine.toFixed(2)} kraadi`);
}